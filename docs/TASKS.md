...
- [ ] Key distances by mandi identity, not by name — claimed by: — done looks like: two offers for the same mandi name at different distances both keep their own distance. `recommendation.py:65` builds `distance_by_mandi` keyed on the mandi **name**, so duplicates collapse to one distance and the tie-break sorts on the wrong number.
